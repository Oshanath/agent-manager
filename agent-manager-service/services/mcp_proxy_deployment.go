...
// ctx is accepted on every I/O-performing helper so cancellation can flow through.
// Downstream dependencies (gatewayRepo, deploymentRepo, gatewayEventsService) are
// shared with non-MCP code and do not yet expose ctx-aware variants; the parameter
// is plumbed but currently unused at those boundaries.
func (s *MCPProxyService) autoDeployToActiveGateways(ctx context.Context, proxy *models.MCPProxy, orgName string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to list active gateways: %w", err)
	}
	if len(gateways) == 0 {
		s.logger.Warn("No active gateway found for MCP proxy auto deployment", "proxyID", proxy.UUID, "orgName", orgName)
		return nil
	}
	...
	if len(errs) > 0 {
		return fmt.Errorf("failed to auto deploy MCP proxy: %w", errors.Join(errs...))
	}
	return nil
}
...
func (s *MCPProxyService) buildMCPProxyDeploymentYAML(proxy *models.MCPProxy) (*MCPProxyDeploymentYAML, error) {
	...
	policies := appendMCPBackendAuthPolicy(proxy.Configuration.Policies, upstreamAuth)
