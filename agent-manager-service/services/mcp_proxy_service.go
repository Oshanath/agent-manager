...
func (s *MCPProxyService) Create(ctx context.Context, orgUUID, createdBy string, req *models.MCPProxyDTO) (*models.MCPProxyDTO, error) {
	...
	proxy := &models.MCPProxy{
		...
		Configuration: models.MCPProxyConfig{
			...
			Upstream:     upstream,
			Policies:     convertSpecMCPPoliciesToModel(req.Policies),
			Capabilities: convertSpecMCPCapabilitiesToModel(req.Capabilities),
		},
	}
...
// Delete removes an MCP proxy by handle.
func (s *MCPProxyService) Delete(ctx context.Context, orgUUID, proxyID string) error {
	...
	gatewayIDs := s.gatewayIDsForDeletion(ctx, proxy, orgUUID)
	s.broadcastMCPProxyDeletion(ctx, proxy, gatewayIDs)
	return nil
}
...
func normalizePolicyVersionToMinor(version string) string {
	...
	if len(parts) == 1 {
		...
		if normalizePolicyVersionToMajor(major) == "v"+major {
			return major + ".0"
		}
		return trimmedVersion
	}
	...
	if normalizePolicyVersionToMajor(major) != "v"+major || normalizePolicyVersionToMajor(minor) != "v"+minor {
		return trimmedVersion
	}
	return major + "." + minor
}
...
func convertSpecMCPPoliciesToModel(policies *[]models.MCPPolicy) []models.MCPPolicy {
...
func convertModelMCPPoliciesToSpec(policies []models.MCPPolicy) *[]models.MCPPolicy {
	if len(policies) == 0 {
		return nil
	}
	out := make([]models.MCPPolicy, 0, len(policies))
...
func convertSpecMCPCapabilitiesToModel(capabilities *models.MCPProxyCapabilities) *models.MCPProxyCapabilities {
	if capabilities == nil {
		return nil
	}
...
func convertModelMCPCapabilitiesToSpec(capabilities *models.MCPProxyCapabilities) *models.MCPProxyCapabilities {
	if capabilities == nil {
		return nil
	}
	return &models.MCPProxyCapabilities{
...
func convertModelMCPProxyToSpec(proxy *models.MCPProxy) *models.MCPProxyDTO {
	...
	return &models.MCPProxyDTO{
		Capabilities:   convertModelMCPCapabilitiesToSpec(proxy.Configuration.Capabilities),
		...
		Name:           name,
		Policies:       convertModelMCPPoliciesToSpec(proxy.Configuration.Policies),
