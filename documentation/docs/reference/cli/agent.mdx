...
# Attach a configured LLM provider and MCP proxy
amctl agent create research-bot \
  --display-name "Research Bot" \
  --subtype chat-api \
  --repo-url https://github.com/acme/research-bot \
  --build-type buildpack \
  --language python \
  --language-version 3.12 \
...
| `--mcp-proxy` | string | (none) | Handle of a configured [MCP proxy](./mcp-proxy.mdx) to attach (internal only). |
| `--mcp-url-env` | string | (none) | Env var name for the MCP proxy URL (default: auto-generated). |
| `--mcp-api-key-env` | string | (none) | Env var name for the MCP proxy API key (default: auto-generated). |
...
- [`amctl mcp-proxy`](./mcp-proxy.mdx) — manage MCP proxies you can attach with `--mcp-proxy`
