...
1. At startup the agent reads the list of MCP server URLs from `MCP_SERVER_URLS`.
...
|---|---|---|---|
| `MCP_SERVER_URLS` | Yes | *(empty)* | Comma-separated list of MCP proxy URLs. Injected automatically by Agent Manager when MCP proxies are bound to this agent. |
...
Manager console, the platform automatically populates `MCP_SERVER_URLS` with the
gateway-accessible URLs of the selected proxies.

## Example `MCP_SERVER_URLS`

```
MCP_SERVER_URLS=https://gw.example.com/my-org/search-server/v1.0/mcp,https://gw.example.com/my-org/data-server/v1.0/mcp
```
...
export MCP_SERVER_URLS="https://db720294-98fd-40f4-85a1-cc6a3b65bc9a-prod.e1-us-east-azure.choreoapis.dev/godzilla/mcp-everything-server/v1.0/mcp"
