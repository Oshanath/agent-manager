...
def _build_server_configs(cfg: Config) -> dict[str, dict[str, Any]]:
    ...
    if not cfg.mcp_server_urls or not api_key:
        return {}
...
class MCPAgent:
    ...
    async def start(self) -> None:
        ...
        if not self._cfg.mcp_server_urls:
            log.warning("No MCP server URLs configured; continuing without MCP tools")
        elif not self._cfg.mcp_api_key:
            log.warning("MCP server URLs configured but API key is missing; continuing without MCP tools")
